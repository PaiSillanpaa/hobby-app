...
const ActivePosts = () => {
  ...
  useEffect(() => {
    const fetchPosts = async () => {
      try {
        const res = await fetch(`${baseurl}/api/listing/active`); // backend endpoint
        if (!res.ok) throw new Error("API error");
        const data = await res.json();
        console.log("data", data)
        setPosts(data.activeListings);
      } catch (err) {
        console.error(err);
        const userId = await fetchUserId();
        const fallbackData = await import("../../data/hobbies.json");
        const filteredPosts = fallbackData.default.filter(
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      const res = await fetch(`${baseurl}/listing/${id}/update`, {
        method: "PUT",
